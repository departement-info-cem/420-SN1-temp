...
<TabItem value="avecportee" label="Avec portée">

Lorsque des variables (ou des paramètres de fonction) avec des noms identiques existent dans des portées différentes,
on peut regrouper les variables par portée dans la trace.<br/>
    Pour cela, on ajoute une ligne horizontale en haut de notre tableau pour indiquer les portées.
