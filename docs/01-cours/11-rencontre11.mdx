...
<Tabs queryString="onglet">
    ...
    <TabItem value="pandas" label="🐼 Pandas">
        ...
        Quand on ajout `.mean()`, Pendas prend chaque groupe créé et calcule la moyenne des valeurs pour la spécifiée (ici la colonne tip). \
        Appliquer un calcul sur chaque groupe afin d'obtenir une valeur est nommé une agrégation.  \
        ...
        Si on souhaite obtenir le résultat de plusieurs fonctions d'agréation en même temps, \
        il est possible d'utiliser la fonction `agg` en lui passant une liste de noms de fonctions d'agrégation :

        ```python
        #obtenir par combinaison de sexe et de jour (2 * 7 == 14 groupes)
        #le minimum, le maximum et la moyenne du total de la facture et du pourboire
        df.groupby(["sex","day"])["tip", "total_bill"].agg(['min', 'max', 'mean'])
        ```
