...
<Tabs queryString="onglet">
    ...
    <TabItem value="pandas" label="🐼 Pandas">
        ...
        Prenez le temps d'observer à l'aide de PyCharm le contenu afficher à l'aide de `print` :
        ...
        df["tip_%"] = (df["tip"] / df["total_bill"]) * 100

        # Ajouter une colonne indiquant si la journée fait partie de la fin de semaine
        df["is_weekend"] = df["day"].str.slice(0,1) == "S"  #  str permet de récupérer le contenu de "day" en tant que chaine de caractère.
                                                            #  La méthode slice(start, stop) permet de récupérer qu'une partie d'une chaîne :
        ...
        print(df.head())
        ```
        Cela ajoute une nouvelle colonne `tip_percentage` et `is_weekend` au DataFrame, \
        l'une calculant le pourcentage de pourboire par rapport au montant total de la facture et l'autre indiquant si la journée fait partie de la fin de semaine :
        ```
           total_bill   tip     sex smoker  day    time  size      tip_%  is_weekend
        ...
        <br/>
        La fonction `groupby` sert à définir des groupes (quelle colonne va servir pour regrouper les lignes du dataframe ensemble). \
        Quand on ajout `.mean()`, penda prends chaque groupe créé et calcule la moyenne des valeurs pour la colonne spécifiée (ici la colonne tip). \
        Appliquer un calcul sur chaque groupe obtenir une valeur est nommée une agrégation. \
        Il existe différente fonction d'agrégation :
        ...
        Si on souhaite obtenir de résultat de plusieurs fonctions d'aggréation en même temps, \
        il est possible d'utiliser la fonction `agg` et passer un tableau des noms des fonctions d'aggrégation souhaité

        ```python
        #obtenir par jour, pour le total de la facture et pour le montant du tip, le minimum, le maximum et la moyenne
        df.groupby("day")["tip", "total_bill"].agg(['min', 'max', 'mean'])
        ```

        La fonction `len()` permet de connaitre le nombre de groupe généré par la fonction `groupby()`
        ```python
        groupe_par_jour = df.groupby("day")
        print(f"Nombre de groupe : {len(groupe_par_jour)}") #Affiche 7
        ```
