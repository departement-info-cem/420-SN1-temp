...
Voici comment Python évalue l’instruction #5, étape par étape :
    ...
    <TabItem value="exercices" label="Exercices">
        ...
        1. Réaliser une fonction :
            - Prenant en entrées les 4 nombres représentant les notes
            - Retournant la note finale calculée
        
        2. Appelez cette fonction pour réaliser le calcul avec les notes saisies

        3. Modifier le calcul de la note finale afin de prendre en compte la double sanction
        ...
        ```python
        temps = [0, 6, 12, 18, 24, 30, 36]          #Valeurs en X pour les 2 gênes
        exp_gene_myc = [95, 80, 60, 45, 30, 15, 5]  #Valeurs en Y pour le gêne Myc
        exp_gene_p21 = [5, 20, 40, 55, 70, 85, 95]  #Valeurs en Y pour le gêne p21
        ```
        
        ---
        ### 🤖 Étape B - Expérimenter avec l'aide de ChatGPT! 
        ...
        Ajoutez et relancer le programme à chaque changement afin de bien voir les changements.
        
        ```python
        plt.figure(figsize=(10, 6)) #Mettre n'importe où avant l'instruction plt.show()
        ```
        
        ```python
        plt.tight_layout() #Mettre n'importe où avant l'instruction plt.show()
        ```

        ```python
        plt.grid(True) #Mettre n'importe où avant l'instruction plt.show()
        ```

        ```python
        plt.axvline(x=16, color='gray', linestyle='--', label='Point d\'équilibre') #Mettre n'importe où avant l'instruction plt.show()
        ```

        ```python
        plt.text(17, 48, '⚖️ Équilibre', fontsize=12, color='gray') #Mettre n'importe où avant l'instruction plt.show()
        ```

        ```python
        plt.savefig('image_du_graphique.png') #Remplacez plt.show() par cette instruction, lancer le programme, puis ouvrez le fichier ".png" généré
        ```
