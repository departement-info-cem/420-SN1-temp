...
```
tp1/
├── racine.py       #sert à contenir vos 2 fonctions pour les 2 méthodes différentes de calcul la racine carrée
├── validation.py   #sert à contenir vos tests validant le bon fonctionnement des fonctions du module "racine.py"
└── performance.py  #sert à contenir vos tests de performance des fonctions du module "racine.py"
```
:::
...
Pour **chacune des deux méthodes**, vous devez créer une fonction dans le module `racine.py` prenant un nombre en paramètre et retournant sa racine carrée en utilisant cette méthode. **Les 2 fonctions doivent recevoir** à ce stade **un seul paramètre** : **le nombre**.
...
Mettez la précision de votre choix dans une variable à l'intérieur de vos fonctions <i>(comme une constante, ex : PRECISION = 8)</i>.

:::tip ✅ Tests attendus

Dans le script `validation.py`, importez le module `racine` puis tester vos 2 fonctions à l'aide d'appels de fonction similaire à ceux ci-dessous :
...
- Ajouter un paramètre `base` à vos fonctions (ex. : 2 pour racine carrée, 3 pour racine cubique, etc.) ce paramètre doit être optionnel et avoir par défaut la valeur 2.<br />
- La racine carrée d'un nombre négatif doit être possible uniquement si la base est impaire (ex. : la racine cubique de -27 est -3).
