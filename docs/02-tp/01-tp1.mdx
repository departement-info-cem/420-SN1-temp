...
Pour un **n** entier positif, faire une fonction qui permet de calculer la racine <KaTeX>{"\\sqrt[n]{x}"}</KaTeX>.
...
- si on part avec le <KaTeX>{"\\log_{2}{64}"}</KaTeX> on trouve 6, car <KaTeX>{"2^6=64"}</KaTeX>
- si on prend le <KaTeX>{"\\log_{2}{2^x}"}</KaTeX> cela donne toujours 1
