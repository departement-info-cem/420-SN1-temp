...
:::warning <Highlight color="tip"><span style={{color: 'white'}}>2 points</span></Highlight> Répertoire Git et documentation des fonctions
...
- Les fonctions doivent être documetées à l'aide d'un [docstring](/cours/rencontre3?onglet=comments) dans un français sans faute : description de la fonction, paramètres, limites et exceptions possibles
:::

::::note <Highlight color="tip"><span style={{color: 'white'}}>5 points</span></Highlight> 🔧 Calculer la racine carrée « à la main »
...
<Row>
    ...
    <Column>
        ...
        \
        \
        Si on souhaite 4 décimales de précision, 
        on devrait alors arrêter quand la différence entre les 2 intervalles est inférieure à 0.00001
        \
        \
            Début : [0, 8] → 8 - 0 = 8 ... c'est > 0.00001 → on continue ... \
                    nouvel intervalle [0, 4] → 4 - 0 = 4 ... c'est > 0.00001 → on continue \
                    nouvel intervalle [2, 4] → 4 - 2 = 2 ... c'est > 0.00001 → on continue \
                    ... \
                    nouvel intervalle [2.828125, 2.8291015625] → \
                    2.8291015625 - 2.828125 = 0.0009765625... c'est < 0.00001 → on arrête
                    
                    La réponse est la moyenne des 2 intervalles.
        ...
        \
        La **précision** doit être **entre 4 et 14 décimales** à votre discrétion. Supérieur à 14 décimales, le calcul prend trop de temps et le programme peut arrêter de fonctionner.
        ...
        \
        **Remarque** : les dernières décimales peuvent variées selon le niveau de présion codée dans vos fonctions. Les résultats affichés sont les résultats de math.sqrt()
...
<Row>
    <Column>
        ...
        Vous allez comparer vos 2 méthodes avec `math.sqrt` pour comparer les performances. Pour chaque méthode (fonction 1, fonction 2, math.sqrt) vous devez :

        - Utiliser une boucle différente par méthode :
            - Réaliser 100 000 appels de la méthode
            ...
            - Calculer le temps nécessaire pour réaliser les 100 000 appels (en ms)
        - Afficher :
            - Le **temps total (en millisecondes)** pour chaque méthode
            - Le **temps moyen** par appel de fonction pour chaque méthode
        :::
    </Column>
    <Column>
        :::tip <Highlight color="tip"><span style={{color: 'white'}}>1 points</span></Highlight> 🌱 Généraliser à une racine n-ième
        ...
        \
        Vous devez rajouter un paramètre `base` (ex. : 2 pour racine carrée, 3 pour racine cubique, etc.).
